    ...
    class User
    {
        ...
        public function advisorRegister($data,$photo)
        {
            ...
            if($this->db->execute())
            {
                ...
                if($this->db->rowCount() > 0)
                {
                    $sql2 = "INSERT INTO advisor (advisor_id,name,address,email,nic_no,tel_no,qualification,photo,is_registered,dob) 
                            VALUES (:id,:name,:address,:mail,:nic,:tel,:qualification,:photo,:status,:dob)";
